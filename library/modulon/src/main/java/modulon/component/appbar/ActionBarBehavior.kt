...
class ActionBarBehavior(var scrollEnabled: Boolean = true) : HeaderBehavior<AppbarView>() {
    ...
    override fun onStartNestedScroll(coordinatorLayout: CoordinatorLayout, child: AppbarView, directTargetChild: View, target: View, axes: Int, type: Int): Boolean {
    ...
    override fun onNestedPreScroll(coordinatorLayout: CoordinatorLayout, child: AppbarView, target: View, dx: Int, dy: Int, consumed: IntArray, type: Int) {
//        if (!scrollEnabled || isLocked) return super.onNestedPreScroll(coordinatorLayout, child, target, dx, dy, consumed, type)
        ...
        if ((dy > 0 && child.isOnTop) || (dy < 0 && (child.isOnBottom || !(target is LazyListView && target.isOnTop)))) {
            return super.onNestedPreScroll(coordinatorLayout, child, target, dx, dy, consumed, type)
        }
    ...
    override fun onNestedScroll(coordinatorLayout: CoordinatorLayout, child: AppbarView, target: View, dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, type: Int, consumed: IntArray) {
    }

    override fun onStopNestedScroll(coordinatorLayout: CoordinatorLayout, child: AppbarView, target: View, type: Int) {
    ...
    private fun animateToPosition(actionBar: AppbarView, up: Boolean) {
    ...
    fun animateToPositionLock(actionBar: AppbarView, up: Boolean) {
        scrollEnabled = !up
        offsetAnimator?.cancel()
        val targetValue = if (up) 64.dp else 0
        val span = abs(targetValue - actionBar.translation) / 64.dpf
    ...
    private fun moveActionBar(actionBar: AppbarView, translation: Int) {
