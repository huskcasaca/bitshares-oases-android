...
//    val memoKeyAuths = account.map { it?.memoKeyAuths.orEmpty() }.distinctUntilChanged()
    ...
    private val userInternal = combineNonNull(userUid, chainId).switchMap { (userUid, chainId) -> LocalUserRepository.getUserLive(userUid, chainId) }
...
//    }
    ...
    val isAccountObservable = accountUid.switchMap { LocalUserRepository.getUserLive(it, ChainPropertyRepository.chainId) }.map { it == null }
