...
class WalletManagerViewModel(application: Application) : BaseViewModel(application) {
    ...
    val isWalletUnlocked = SecurityService.isUnlocked
    ...
    fun unlock(): Boolean {
        return (SecurityService.unlock(password)).also {
            isPasswordFieldError.value = !it
        }
    }

    fun change(): Boolean {
        return (unlock() && (newPassword.length >= PASSWORD_LENGTH_MIN).also { isNewPasswordFieldError.value = !it } && (repeatPassword.length >= PASSWORD_LENGTH_MIN && newPassword == repeatPassword).also { isRepeatPasswordFieldError.value = !it } && SecurityService.changePassword(newPassword))
    }
    ...
    fun importBackup() {
        blockchainDatabaseScope.launch {
            LocalUserRepository.add(users.value)
        }
    }
...
//                        val keys = bin.privateKeys
    ...
    fun buildBackup() {
        viewModelScope.launch(Dispatchers.IO) {
            val block = LocalUserRepository.createBackup()
