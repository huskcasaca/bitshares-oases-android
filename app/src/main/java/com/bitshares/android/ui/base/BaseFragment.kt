...
import com.bitshares.android.netowrk.java_websocket.WebSocketState
...
abstract class BaseFragment : UnionFragment() {
    ...
    fun ActionBarLayout.walletStateMenu() = menu {
        ...
        icon = if (SecurityService.isUnlocked.value) unlocked else locked
        locked.doOnAnimationEnd {
            icon = unlocked
            locked.reset()
            if (!SecurityService.isUnlocked.value) unlocked.start()
        }
        unlocked.doOnAnimationEnd {
            icon = locked
            unlocked.reset()
            if (SecurityService.isUnlocked.value) locked.start()
        }
        ...
        doOnClick { if (SecurityService.isUnlocked.value) SecurityService.lock() else lifecycleScope.launch(Dispatchers.Main.immediate) { startWalletUnlock() } }
