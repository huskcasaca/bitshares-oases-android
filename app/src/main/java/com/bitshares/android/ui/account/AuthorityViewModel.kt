package com.bitshares.android.ui.account
...
open class AuthorityViewModel(application: Application) : AccountViewModel(application) {
    ...
    val isOwnerSufficientLocal = combineFirstOrNull(ownerKeyAuths, ownerKeyAuthsLocal, ownerMinThreshold, SecurityService.isUnlocked) { keyAuths, keyAuthsLocal, threshold, unlocked -> if (unlocked) checkSufficient(keyAuths, keyAuthsLocal, threshold) else null }
    val isActiveSufficientLocal = combineFirstOrNull(activeKeyAuths, activeKeyAuthsLocal, activeMinThreshold, SecurityService.isUnlocked) { keyAuths, keyAuthsLocal, threshold, unlocked -> if (unlocked) checkSufficient(keyAuths, keyAuthsLocal, threshold) else null }
    val isMemoSufficientLocal = combineFirstOrNull(memoKeyAuths, memoKeyAuthsLocal, memoMinThreshold, SecurityService.isUnlocked) { keyAuths, keyAuthsLocal, threshold, unlocked -> if (unlocked) checkSufficient(keyAuths, keyAuthsLocal, 1U) else null }
