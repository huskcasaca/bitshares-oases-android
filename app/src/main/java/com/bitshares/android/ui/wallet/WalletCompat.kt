...
import com.bitshares.android.chain.Clipboard
import com.bitshares.android.chain.blockchainDatabaseScope
import com.bitshares.android.chain.walletPasswordFilter
import com.bitshares.android.database.entities.User
import com.bitshares.android.extensions.text.StringFilter
import com.bitshares.android.extensions.text.createStringFilterHint
import com.bitshares.android.extensions.text.validateStringFilter
import com.bitshares.android.extensions.viewbinder.bindUserV1
...
suspend fun Union.showWalletPasswordUnlockDialog() = showBooleanSuspendedBottomDialog {
    if (SecurityService.unlock()) dismissWith(true)
...
suspend fun Union.showWalletBiometricUnlockDialog() = if (SecurityService.unlock()) true else showBooleanSuspendedBottomDialog {
    ...
    val cipher = try {
        SecurityService.Provider.bioSecureProvider.initDecryptionCipher(SecurityService.Provider.fingerprintNonceBytes)
    ...
    bioManager.startAuthentication(service, FingerprintManager.CryptoObject(cipher)) {
        ...
        doOnAuthenticationSucceeded {
            dismissWith(SecurityService.unlockFingerprint())
        }
...
suspend fun Union.showWalletBiometricSetupDialog() = showBooleanSuspendedBottomDialog {
    if (!SecurityService.unlock()) dismissWith(false)
    val bioManager = FingerprintAuthentication(context)
    val service = getSystemService<FingerprintManager>() ?: return@showBooleanSuspendedBottomDialog dismissWith(false)
    title = context.getString(R.string.wallet_unlock_wallet_title)
    message = context.getString(R.string.fingerprint_touch_sensor)
    ...
    val cipher = try {
        SecurityService.Provider.bioSecureProvider.initEncryptionCipher()
    } catch (e: RuntimeException) {
        SecurityService.disableFingerprint()
        try {
            SecurityService.Provider.bioSecureProvider.initEncryptionCipher()
    ...
    bioManager.startAuthentication(service, FingerprintManager.CryptoObject(cipher)) {
        ...
        doOnAuthenticationSucceeded {
            SecurityService.enableFingerprint()
            dismissWith(true)
        }
        doOnAuthenticationError { errorCode, errString ->
            message = errString.toString()
...
suspend fun Union.startWalletUnlock() = when {
    SecurityService.isCorrupted -> showWalletCorruptedResetDialog()
    !SecurityService.unlock() -> if (Settings.KEY_USE_FINGERPRINT.value) showWalletBiometricUnlockDialog() else showWalletPasswordUnlockDialog()
    else -> true
}
...
fun Union.showWalletResetDialog() = showBottomDialog {
    ...
    button {
        text = context.getString(R.string.button_reset)
        textColor = context.getColor(R.color.component_error)
        doOnClick { SecurityService.reset() }
    }
    button { text = context.getString(R.string.button_cancel) }
}
