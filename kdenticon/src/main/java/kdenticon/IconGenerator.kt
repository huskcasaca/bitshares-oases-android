...
class IconGenerator(renderer: SvgRenderer, hash: String, x: Float, y: Float, size: Float, padding: Float?, config: Config) {
    ...
    // Calculate padding
    private var _padding = floor(size * (if(padding != null) padding else 0f))
    private var _size = size - _padding * 2f
    ...
    private var _x = x + floor(_padding + _size / 2f - cell * 2f);
    private var _y = y + floor(_padding + _size / 2f - cell * 2f);
    ...
    fun renderShape(
        ...
        for (i in positions.indices) {
            graphics.noTransform = Transform(
    ...
    fun isDuplicate(values: List<Int>) : Boolean {
        if (values.indexOf(index) >= 0) {
            for (element in values) {
                if (selectedColorIndexes.indexOf(element) >= 0) {
                    return true
                }
            }
        }
